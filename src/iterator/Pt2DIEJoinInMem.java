...
public class Pt2DIEJoinInMem {
	...
	private Tuple Sorted1[], Sorted2[];


	public Pt2DIEJoinInMem(String r1, FldSpec[] r1_fields, AttrType[] r1_types, int r1_size, String r2, FldSpec[] r2_fields, AttrType[] r2_types, int r2_size,
					...
					throws JoinsException, IndexException, InvalidTupleSizeException, InvalidTypeException, PageNotReadException, PredEvalException, LowMemException, UnknowAttrType, UnknownKeyTypeException, Exception
	{
		...
		Sorted1 = new Tuple[ntsize];
		Sorted2 = new Tuple[ntsize];
			...
			if(updown1.tupleOrder == TupleOrder.Ascending)
			{
				if(TempTuple1.getIntFld(r1c1) <= TempTuple2.getIntFld(r2c1))
				{
					try {
						if(tt1 == null)
						{
							Sorted1[tableArrIndex] = new Tuple(TempTuple1);
				...
				else
				{
					...
					try {
						if(tt2 == null)
						{
							Sorted1[tableArrIndex] = new Tuple(TempTuple2);
				...
				if(updown2.tupleOrder == TupleOrder.Ascending)
				{
					if(TempTuple1.getIntFld(r1c2) <= TempTuple2.getIntFld(r2c2))
					{
						...
						try {
							if(tt1 == null)
							{
								Sorted2[tableArrIndex] = new Tuple(TempTuple1);
					...
					else
					{
						try {
							if(tt2 == null)
							{
								Sorted2[tableArrIndex] = new Tuple(TempTuple2);
		...
		if(cond[1] != null)
		{
			...
			Tuple t1 = Sorted1[0];//scan1.get_next();
			Tuple t2 = Sorted2[0];//scan2.get_next();
			int inindex = 0;
			int outindex = 0;

			for(;outindex < ntsize; outindex++)
			{
				int position = 0;
				for(inindex = 0; inindex < ntsize; inindex++)
				{
					byte[] temp1 = Sorted1[outindex].getData();
					byte[] temp2 = Sorted2[inindex].getData();
					if (Arrays.equals(temp1, temp2)) 
					{
						inindex = ntsize;
					}
				...
				for(;innerindex < ntsize; innerindex++)
				{
					...
					else
					{
						...
						if(bitArr[innerindex] == 1 && !tableArr1[outerindex] && tableArr2[innerindex])
						{
							...
							try{
								//get tuple L1[i],L2[j]
								Tuple t1 = Sorted1[outerindex];
								Tuple t2 = Sorted2[innerindex];
